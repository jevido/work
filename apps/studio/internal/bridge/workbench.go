// Package bridge holds the thin adapters the frontend can call.
//
// Named for the seam rather than for Wails' word for it. These are registered
// as Wails "services", but the repository now uses that word for a deployed
// program with no interface -- services/sync, services/charted -- and this is
// the opposite: app-private glue that only exists because a webview cannot call
// Go directly. The frontend's half of the same seam is frontend/src/lib/bridge.
//
...
import (
	...
	"dev.jevido/work/apps/studio/internal/changes"
	"dev.jevido/work/apps/studio/internal/charted"
	"dev.jevido/work/apps/studio/internal/claude"
	"dev.jevido/work/apps/studio/internal/config"
	"dev.jevido/work/apps/studio/internal/workbench"
)
...
type PublishResult struct {
	...
	Broken []charted.Link `json:"broken"`
}
...
func (s *WorkbenchService) DocumentationSite() DocumentationSite {
	cfg, err := config.Load()
	if err != nil || cfg.Charted == nil {
		return DocumentationSite{}
	}
	return DocumentationSite{
		BaseURL:  cfg.Charted.BaseURL,
		Space:    cfg.Charted.Space,
		Writable: strings.TrimSpace(cfg.Charted.Token) != "",
	}
}
...
func (s *WorkbenchService) PublishPage(slug, title, description, markdown string) (PublishResult, error) {
	cfg, err := config.Load()
	if err != nil {
		return PublishResult{}, err
	}
	if cfg.Charted == nil {
		return PublishResult{}, charted.ErrNoSite
	}

	client, err := charted.New(cfg.Charted.BaseURL, cfg.Charted.Token)
	if err != nil {
		return PublishResult{}, err
	}

	space := strings.TrimSpace(cfg.Charted.Space)
	slug = strings.Trim(strings.TrimSpace(slug), "/")
	if space == "" {
		return PublishResult{}, errors.New("no documentation space is set, so there is nowhere for this page to go")
	}
	if slug == "" || strings.TrimSpace(title) == "" {
		return PublishResult{}, errors.New("a page needs a slug and a title")
	}
	if len(markdown) > maxPromptBytes {
		return PublishResult{}, errors.New("that page is too long to publish in one piece")
	}

	ctx, cancel := context.WithTimeout(context.Background(), joinTimeout)
	defer cancel()

	broken, err := client.Publish(ctx, charted.Page{
		Space:       space,
		Slug:        slug,
	...
	return PublishResult{
		URL:    strings.TrimSuffix(cfg.Charted.BaseURL, "/") + "/" + space + "/" + slug,
		Broken: broken,
	}, nil
}
