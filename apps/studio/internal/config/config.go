...
type Config struct {
	...
	// Charted is where documentation mode publishes to.
	//
	// Local, not in the workspace, and for a blunter reason than taste: it
	// carries a write token. A workspace is shared with everyone who has its
	// key, and a token that travels with it is a token every reader could
	// publish with.
	Charted *Charted `json:"charted,omitempty"`
}

// Charted is one documentation site: where it is, and what may write to it.
type Charted struct {
	// BaseURL is the site's origin, as https://docs.example.com. No path.
	BaseURL string `json:"baseUrl"`

	// Token is the bearer token the site's write side wants. Empty means this
	// machine can read the site but not publish to it, which is exactly what
	// happens on a machine somebody has not given a token to.
	Token string `json:"token,omitempty"`

	// Space is the section pages land in when nothing names one -- normally
