...
func TestRestructureRefusals(t *testing.T) {
	...
	t.Run("an empty request", func(t *testing.T) {
		if _, err := w.Restructure("tab1", ModeOrientation, "   "); err == nil {
			t.Error("an empty request was accepted")
		}
	})

	t.Run("implementation mode has nothing to restructure", func(t *testing.T) {
		_, err := w.Restructure("tab1", ModeImplementation, "tidy this up")
		if err == nil {
			t.Fatal("implementation mode was accepted")
		}
		if !strings.Contains(err.Error(), ModeImplementation) {
...
func TestProposalPrompt(t *testing.T) {
	state := StateBlock(sampleDoc(), "tab1", ModeOrientation)

	t.Run("orientation mode", func(t *testing.T) {
		got := proposalPrompt(&proposalRun{state: state, mode: ModeOrientation}, "group these by area")
	...
	t.Run("isolation mode says what a plan is", func(t *testing.T) {
		planning := StateBlock(sampleDoc(), "tab1", ModeIsolation)
		got := proposalPrompt(&proposalRun{state: planning, mode: ModeIsolation}, "break this up")
		if !strings.Contains(got, "ordered list of tasks") {
			t.Errorf("isolation mode is not described\n---\n%s", got[:200])
		}
	...
	t.Run("the request is last", func(t *testing.T) {
		...
		got := proposalPrompt(&proposalRun{state: state, mode: ModeOrientation}, "UNIQUEMARKER")
