...
// the sentence somebody wrote in isolation.
...
// A colleague closing a task in isolation closes the card here. The reverse --
...
func TestATaskClosedElsewhereClosesItsCard(t *testing.T) {
	...
	if got.Status != board.StatusDone {
		t.Fatalf("card status = %q after the task was closed in isolation, want %q", got.Status, board.StatusDone)
	}
}
...
func TestClearConversationLeavesTheBoard(t *testing.T) {
	...
	w.ClearConversation(in(ModeImplementation))
...
func TestClearConversationLeavesTheOthers(t *testing.T) {
	...
	here := Conversation{TabID: "tab-1", Mode: ModeOrientation}
	sameTab := Conversation{TabID: "tab-1", Mode: ModeImplementation}
	otherTab := Conversation{TabID: "tab-2", Mode: ModeOrientation}
...
// from, which is where the next isolation pass will see it.
