...
func TestReloadAgentsPicksUpFolderChanges(t *testing.T) {
	...
	// However many a fresh root seeds -- two guaranteed agents and the starter
	// colleagues beside them. The number is agents.Ensure's business; what this
	// test is about is the two changes below moving it by one each way.
	seeded := len(list)
	if seeded < 2 {
		t.Fatalf("got %d agents, want at least the two built-ins", seeded)
	}
	...
	if len(list) != seeded+1 {
		t.Fatalf("got %d agents after adding a folder, want %d", len(list), seeded+1)
	}
	...
	if len(list) != seeded {
		t.Fatalf("got %d agents after removing a folder, want the %d it started with",
			len(list), seeded)
	}
}
...
func TestReloadAgentsReconcilesSessions(t *testing.T) {
	...
	w.rememberSession(runSession("anton", in(ModeImplementation)), "session-anton")
	w.rememberSession(runSession("temp", in(ModeImplementation)), "session-temp")
	...
	if got := w.sessionFor(runSession("anton", in(ModeImplementation))); got != "session-anton" {
		t.Errorf("surviving agent lost their session: %q", got)
	}
	if got := w.sessionFor(runSession("temp", in(ModeImplementation))); got != "" {
...
// And the roster setup produces can route, which is a different property: the
// routing schema enumerates the non-coordinators, and a fresh root has to have
// some.
//
// It did not always. A fresh root used to be a coordinator and an adviser, and
// an adviser is never given a step -- so there was nobody to delegate to on a
// new install, and the first thing anybody saw was a team that could only ever
// answer for itself. The starter colleagues in templates/agents are what
// changed that, so this is the test that says why they ship.
func TestAFreshRootCanDelegate(t *testing.T) {
	w := newTestWorkbench(t)
	...
	if got := specialistIDs(w.registry); len(got) == 0 {
		t.Error("a fresh root has nobody Anton can hand a step to")
	}
	if _, err := planSchema(w.registry); err != nil {
		t.Errorf("planSchema on a fresh root: %v", err)
	}
}
...
func TestConversationsAreLedByTheirModeOwner(t *testing.T) {
	...
	for mode, want := range map[string]string{
		ModeOrientation:    "jared",
		ModeIsolation:      "jared",
		ModeImplementation: "anton",
	} {
