...
func TestChatSessionIsSeparateFromRunSession(t *testing.T) {
	...
	w.rememberSession(runSession("anton", in(ModeImplementation)), "run-session")
	w.rememberSession(chatSession("anton", in(ModeImplementation)), "chat-session")

	if got := w.sessionFor(runSession("anton", in(ModeImplementation))); got != "run-session" {
		t.Errorf("run session = %q, want %q", got, "run-session")
	}
	if got := w.sessionFor(chatSession("anton", in(ModeImplementation))); got != "chat-session" {
	...
	w.forgetSession(runSession("anton", in(ModeImplementation)))
	if got := w.sessionFor(chatSession("anton", in(ModeImplementation))); got != "chat-session" {
...
func TestChatSessionCannotBeForgedByAgentName(t *testing.T) {
	...
	w.rememberSession(chatSession("anton", in(ModeImplementation)), "anton-chat")
	// A folder called "anton#chat" is a legal folder.
	w.rememberSession(runSession("anton#chat", in(ModeImplementation)), "impostor")

	if got := w.sessionFor(chatSession("anton", in(ModeImplementation))); got != "anton-chat" {
...
func TestChatRefusesASecondQuestion(t *testing.T) {
	...
	if _, err := w.Chat(in(ModeImplementation), "and another thing"); err == nil {
...
func TestChatIsAcceptedWhileARunIsActive(t *testing.T) {
	...
	if _, err := w.Submit(in(ModeImplementation), "", "and another thing"); err == nil {
		t.Fatal("Submit accepted a second run; the limit it works around is gone")
	}

	task, err := w.Chat(in(ModeImplementation), "how far along is Chris?")
	if err != nil {
...
func TestReloadKeepsChatSessionForSurvivingAgent(t *testing.T) {
	...
	w.rememberSession(chatSession(coordinator.ID, in(ModeImplementation)), "chat-session")
	w.rememberSession(chatSession("departed", in(ModeImplementation)), "gone-session")

	if _, err := w.ReloadAgents(); err != nil {
		t.Fatalf("ReloadAgents: %v", err)
	}

	if got := w.sessionFor(chatSession(coordinator.ID, in(ModeImplementation))); got != "chat-session" {
		t.Errorf("chat session = %q after reload, want it kept", got)
	}
	if got := w.sessionFor(chatSession("departed", in(ModeImplementation))); got != "" {
...
func TestChatSessionsAreSplitByMode(t *testing.T) {
	...
	w.rememberSession(chatSession("anton", in(ModeOrientation)), "about-shape")
	w.rememberSession(chatSession("anton", in(ModeIsolation)), "about-tasks")
	w.rememberSession(chatSession("anton", in(ModeImplementation)), "about-the-run")

	for _, want := range []struct{ mode, session string }{
		{ModeOrientation, "about-shape"},
		{ModeIsolation, "about-tasks"},
		{ModeImplementation, "about-the-run"},
	} {
	...
	if a, b := chatSession("anton", in(ModeOrientation)), chatSession("anton", in(ModeOrientation)); a != b {
	...
	if runSession("anton", in(ModeImplementation)) == chatSession("anton", in(ModeImplementation)) {
...
//
// Two tabs both in implementation mode used to share a transcript and a session, so a
...
func TestConversationsAreSplitByTab(t *testing.T) {
	...
	here := Conversation{TabID: "tab-1", Mode: ModeImplementation}
	there := Conversation{TabID: "tab-2", Mode: ModeImplementation}
	...
	forged := Conversation{TabID: "tab-1\x00work", Mode: ModeImplementation}
