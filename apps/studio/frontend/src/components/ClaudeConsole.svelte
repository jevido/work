<script lang="ts">
  ...
  import { followTail } from "@ui/follow";
   ...
   * from you: implementation mode takes an instruction and orientation mode takes a thought. A
  ...
  // The modes that start runs, as opposed to the two that reshape a document.
  const isWork = $derived(
    session.mode === "implementation" || session.mode === "documentation",
  );
