<script lang="ts">
  ...
  import OrientationOutline from "./components/OrientationOutline.svelte";
  ...
  import IsolationList from "./components/IsolationList.svelte";
  import DocumentationList from "./components/DocumentationList.svelte";
  ...
  import { MODE_LABELS, MODES, labelOf } from "./lib/workspace/model";
  ...
  const session = $derived(conversations.for(active?.id ?? "", active?.mode ?? "implementation"));
  ...
  const reviewing = $derived(
    proposals.open &&
      ...
      (active.mode === "orientation" || active.mode === "isolation"),
  );
  ...
  const waiting = $derived(
    proposals.open &&
      active !== null &&
      proposals.workspaceId === active.id &&
      active.mode !== "orientation" &&
      active.mode !== "isolation",
  );
  ...
  */
  ...
  function onKeydown(event: KeyboardEvent) {
    ...
    // point of the modes is that they are the same work seen several ways,
    ...
    <div class="body">
        ...
        {#if active}
          <div class="modebar">
            ...
            <!--
              ...
              says so is a better answer than a hint about what Orientation mode is.
            -->
          ...
          {#if waiting && active}
            <div class="refused" role="status">
              ...
              <button onclick={() => (active.mode = proposals.mode)}>
                Review them in {MODE_LABELS[proposals.mode]}
              </button>
            </div>
          ...
          <!--
            The modes, stacked in one grid cell.
            ...
            Orientation, isolation and documentation have nothing running behind them. Their state is
          ...
          <div class="panes">
            ...
            {#key active.id}
              {#if active.mode === "orientation" || active.mode === "isolation"}
                ...
                <div class={["pane", "split", { reviewing }]}>
                  <div class="doc">
                    {#if active.mode === "orientation"}
                      <OrientationOutline workspace={active} />
                    {:else}
                      <IsolationList workspace={active} />
                    {/if}
                  </div>
                  {#if reviewing}
              ...
              {:else if active.mode === "documentation"}
                <div class="pane">
                  <DocumentationList workspace={active} />
                </div>
              {/if}
            {/key}

            <div class="pane work" class:hidden={active.mode !== "implementation"}>
              <!--
              ...
              <div class="office">
                ...
                <OfficeCanvas
                  ...
                  {config}
                  {showPerf}
                  shown={active.mode === "implementation"}
                />
              </div>
            </div>
          </div>
      ...
      <aside>
        <!-- The console is outside the mode stack, and stays mounted for the
             ...
             in implementation mode is still going while you are writing an outline, and
        ...
        <ClaudeConsole
          {session}
          {review}
          lead={roster.leadFor(active?.mode ?? "implementation")}
