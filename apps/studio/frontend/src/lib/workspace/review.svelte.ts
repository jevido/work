...
import { findInTree, labelOf, MODE_LABELS, TASK_STATE_LABELS, type Mode } from "./model";
import {
  blockedBecause,
  inverseOf,
  landed,
...
export class Review {
  ...
  take(proposal: Proposal, workspace: Workspace): void {
    ...
    this.mode = workspace.mode === "isolation" ? "isolation" : "orientation";
    ...
    this.said =
      workspace.mode === "implementation"
        ? `${what} They are waiting in ${MODE_LABELS[this.mode]} mode.`
