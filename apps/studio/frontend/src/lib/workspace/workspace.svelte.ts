...
import type { Document, Edit } from "../../../bindings/dev.jevido/work/apps/studio/internal/workbench/models.js";
...
export class Workspace {
   ...
   * you left in isolation is a tab you were taking something apart in; coming
   * back to it in orientation mode means finding your place again on every
   * switch, which is the one thing tabs are supposed to save you.
   */
  mode = $state<Mode>("orientation");
  ...
  constructor(id: string, snapshot: OutlineSnapshot | null) {
    this.id = id;
    this.mode = (snapshot && modeFrom(snapshot.mode)) ?? "orientation";
