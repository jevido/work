...
import { MODES, type Mode } from "../workspace/model";
...
export class Conversations {
   ...
   * an event arrives. A restructuring asked for in isolation answers into
   * isolation's transcript even if somebody has switched to implementation by the time it
  ...
  current: () => ClaudeSession = () => this.for("", "implementation");
  ...
  for(tab: string, mode: Mode): ClaudeSession {
    ...
    if (!byMode) {
      // Built from MODES rather than listed here, so a mode added to the
      // model gets a transcript without a second place to remember.
      byMode = Object.fromEntries(
        MODES.map((mode) => [mode, named(tab, mode)]),
      ) as Record<Mode, ClaudeSession>;
  ...
  all(): ClaudeSession[] {
    ...
    for (const byMode of this.#sessions.values()) {
      for (const mode of MODES) out.push(byMode[mode]);
    }
    return out;
  }
  ...
  forget(tab: string): void {
    ...
    for (const mode of MODES) {
      const session = byMode[mode];
      if (session.busy || session.chatBusy) return;
    }
    for (const mode of MODES) byMode[mode].stop();
