...
export class ClaudeSession {
  ...
  async submit(prompt: string, agentId = ""): Promise<void> {
    ...
    // Orientation and isolation do not start runs. A run is Claude let loose
    // on a working tree; what those two modes want is Claude let loose on the
    ...
    // right: it was a composer for work, shown in every mode, and in the two
    // that reshape a document it reached an agent who could not see the map
    // and could not change it. See reshape.
    if (this.mode === "orientation" || this.mode === "isolation") {
