...
async function run() {
  ...
  // A mode switch. Orientation and isolation mount and unmount; implementation keeps its box.
  pickMode("orientation");
  await settle(20);
  check("switching to orientation draws the outline", !!$(".idea"));
  check(
  ...
  check(
    ...
    `${paintedAfter - paintedAtSwitch} draw calls over 45 frames of orientation mode`,
  );

  // And it starts again when it is shown, or the check above proves nothing.
  pickMode("implementation");
  await settle(20);
  check(
  ...
  pickMode("isolation");
  await settle(20);
  check("isolation is the checked mode before switching tabs", checkedMode() === "isolation");
  ...
  check("orientation mode offers a way to ask", !!askButton(), askButton()?.textContent?.trim() ?? "no button");
  ...
  check("orientation mode offers both views", !!viewRadio("Outline") && !!viewRadio("Map"));
