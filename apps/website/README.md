...
Nothing here issues a request other than `GET`. There is no `POST` in the
...
The read half of the API in `services/sync/README.md`, at the origin that served
the page:
...
`ops.ts` is itself a port of `packages/ops`, which is the normative
...
The footer, under a board, offers the desktop binary:
`components/Install.svelte`.
...
npm run build          # static files in apps/website/dist
npm run check          # svelte-check
```

`npm run build` writes `apps/website/dist`, and that directory is what the sync
server serves at `/`. In production the image builds it:
`infra/sync/Dockerfile`'s `site` stage runs `npm run check && npm run build`
here and copies the result to `/srv/site`, which is what `WORK_SITE_DIR` points
at. Nothing has to be built and uploaded separately, and `docker build .` on a
laptop produces the same image as the deploy.

Served by hand rather than by `http.FileServer`, in
`services/sync/api/static.go`: anything that is not a file in the directory
falls through to `index.html` at 200, so reloading a route the client owns
works, and `/v1/…` is registered ahead of the root so a mistyped endpoint is
still a JSON 404 rather than this page. Locally,
`WORK_SITE_DIR=../../apps/website/dist` from `services/sync` does the same thing
without a container.
